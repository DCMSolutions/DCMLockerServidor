...
namespace DCMLockerServidor.Server.Repositorio.Implementacion
{
    public class TokenRepositorio : ITokenRepositorio
    {
        ...
        public async Task<bool> VerifyToken(Token token)
        {
            ...
            Console.WriteLine("token.Cantidad" + token.Cantidad);
            Console.WriteLine("token.Contador" + token.Contador);
            //Console.WriteLine("token.Cantidad<token.Contador" + token.Cantidad < token.Contador);
        ...
        public async Task<int> ConfirmarCompraToken(int idToken)
        {
            ...
            {
                List<Token> tokens = await GetTokensValidosByLockerFechasSize(token.IdLocker.Value, token.IdSize.Value, DateTime.Now, DateTime.Now,"Por fecha");
